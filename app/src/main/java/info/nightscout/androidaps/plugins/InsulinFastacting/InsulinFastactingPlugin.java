...
public class InsulinFastactingPlugin implements PluginBase, InsulinInterface {
    ...
    public Iob iobCalcForTreatment(Bolus bolus, long time, Double dia) {
        ...
        if (bolus.insulinAmount != 0d) {
            ...
            if (minAgo < peak) {
                double x1 = minAgo / 5d + 1;
                result.iobContrib = bolus.insulinAmount * (1 - 0.001852 * x1 * x1 + 0.001852 * x1);
                // units: BG (mg/dL)  = (BG/U) *    U insulin     * scalar
                result.activityContrib = bolus.insulinAmount * (2 / dia / 60 / peak) * minAgo;

            } else if (minAgo < end) {
                double x2 = (minAgo - 75) / 5;
                result.iobContrib = bolus.insulinAmount * (0.001323 * x2 * x2 - 0.054233 * x2 + 0.55556);
                result.activityContrib = bolus.insulinAmount * (2 / dia / 60 - (minAgo - peak) * 2 / dia / 60 / (60 * 3 - peak));
            }
        }
        return result;
    }
