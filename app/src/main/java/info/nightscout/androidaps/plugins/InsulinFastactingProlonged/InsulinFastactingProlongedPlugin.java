...
public class InsulinFastactingProlongedPlugin implements PluginBase, InsulinInterface {
    ...
    public Iob iobCalcForTreatment(Bolus bolus, long time, Double dia) {
        ...
        if (bolus.insulinAmount != 0d) {
            long bolusTime = bolus.date;
            double minAgo = (time - bolusTime) / 1000d / 60d;

            if (minAgo < peak) {
                double x1 = 6 / dia * minAgo / 5d + 1;
                result.iobContrib = bolus.insulinAmount * (1 - 0.0012595 * x1 * x1 + 0.0012595 * x1);
                // units: BG (mg/dL)  = (BG/U) *    U insulin     * scalar
                result.activityContrib = bolus.insulinAmount * ((2 * peak / Total) * 2 / peak / peak * minAgo);
            } else if (minAgo < tail) {
                double x2 = (6 / dia * (minAgo - peak)) / 5;
                result.iobContrib = bolus.insulinAmount * (0.00074 * x2 * x2 - 0.0403 * x2 + 0.69772);
                result.activityContrib = bolus.insulinAmount * (-((2 * peak / Total) * 2 / peak * 3 / 4) / (tail - peak) * (minAgo - peak) + (2 * peak / Total) * 2 / peak);
            } else if (minAgo < end) {
                double x3 = (6 / dia * (minAgo - tail)) / 5;
                result.iobContrib = bolus.insulinAmount * (0.0001323 * x3 * x3 - 0.0097 * x3 + 0.17776);
                result.activityContrib = bolus.insulinAmount * (-((2 * peak / Total) * 2 / peak * 1 / 4) / (end - tail) * (minAgo - tail) + (2 * peak / Total) * 2 / peak / 4);
            }
        }
        return result;
    }
